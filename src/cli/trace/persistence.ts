...
function appendTraceLines(lines: string[]): void {
  const traceFilePath = getOrCreateActiveTraceFilePath();
  if (!traceFilePath) {
    return;
  }
...
/**
 * Lazily allocates the trace transcript path when the first trace line is
 * persisted so configuration-only runs do not emit empty files.
 */
function getOrCreateActiveTraceFilePath(): null | string {
  if (activeTraceFilePath) {
    return activeTraceFilePath;
  }
