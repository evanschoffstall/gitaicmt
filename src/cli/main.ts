...
const HELP_COMMAND_ROWS = [
  ...
  [
    "resume list",
    "List saved plan bundles for the current repository (use --global for all repos)",
  ],
...
const HELP_FLAG_ROWS = [
  ...
  [
    "--enforce-commit-body",
    "Require bullet-body validation when replaying saved plan commits",
  ],
  ["--global", "When listing plan bundles, include all repositories"],
...
function createCommandHandlers(
  ...
  const singleCommitHandler = createSingleCommitHandler(
    ...
    skipTokenCheck,
    options.breakingMode,
    options.hasEnforceCommitBodyFlag,
  );
...
function createResumeHandler(options: CliOptions): CommandHandler {
  return () => {
    ...
    if (options.resumeCommand === "list") {
      return runExecutionCommand(options.outputMode, async () => {
        const { cmdResumeList } = await commandModule;
        cmdResumeList(options.hasGlobalFlag);
      });
    }
  ...
  return () => {
    ...
    if (outputMode === "off") {
      return commandModule.then(({ cmdCommitSingle }) =>
        cmdCommitSingle(skipTokenCheck, breakingMode, enforceMessageBody),
      );
    }

    return runExecutionCommand(outputMode, async () => {
      const { cmdCommitSingle } = await commandModule;
      await cmdCommitSingle(skipTokenCheck, breakingMode, enforceMessageBody);
    });
  };
}
...
async function runExecutionCommand(
  ...
  try {
    ...
    sessionDisplay.flushVerboseAiOutput({ includeCoverageSummary: true });
  } finally {
