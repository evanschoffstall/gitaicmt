...
/**
 * Build the status rows shown before resume execution begins.
 */
...
/**
 * Emit any warnings or skip notices associated with the selected hash-check
 * mode before executing the saved commit plan.
 */
export function logResumeHashCheckMessages(
  ...
  if (invalidCommits.length > 0) {
    const invalidCommitAction = validOnly
      ? `Skipping ${formatCount(invalidCommits.length)} commit(s) whose saved file or hunk hashes no longer match the current staged patch.`
      : `Found ${formatCount(invalidCommits.length)} commit(s) whose saved file or hunk hashes no longer match the current staged patch.`;
    log(`${YELLOW}${invalidCommitAction}${RESET}`);
...
/**
 * Resolve the saved commits that should execute for the chosen resume mode.
 * Default mode is strict: any hash mismatch aborts the resume. `--valid-only`
 * salvages the matching subset, while `--force` bypasses commit-level hash
 * validation after repository-scoped restoration succeeds.
 */
export function resolveResumeExecutionPlan(
  ...
  const pendingIndexes = filterPendingResumeIndexes(bundle, selectedIndexes);
  preparePlanBundleForResume(bundle, pendingIndexes);

  if (forceHashCheck) {
    return {
      invalidCommits: [],
      validIndexes: pendingIndexes,
      validPlan: buildSelectedResumePlan(bundle, pendingIndexes),
    };
  }

  const validatedPlan = filterValidPlanCommitsForResume(bundle, pendingIndexes);
  if (!validOnly && validatedPlan.invalidCommits.length > 0) {
    throw new ValidationError(
      buildStrictResumeValidationMessage(validatedPlan.invalidCommits),
    );
  }
...
function buildSelectedResumePlan(
  bundle: PersistedPlanBundle,
  selectedIndexes: number[],
): ResumeExecutionPlan["validPlan"] {
  return selectedIndexes.map((selectedIndex) => bundle.plan[selectedIndex - 1]);
}

function buildStrictResumeValidationMessage(
  invalidCommits: ResumeExecutionPlan["invalidCommits"],
): string {
  const mismatchLines = invalidCommits.map((invalidCommit) => {
    const subject =
      invalidCommit.message.split("\n")[0] ?? invalidCommit.message;
    return `#${String(invalidCommit.index)} ${subject}: ${invalidCommit.mismatch}`;
  });
  const commitLabel = invalidCommits.length === 1 ? "commit" : "commits";

  return [
    `Cannot resume in strict hash-check mode because ${formatCount(invalidCommits.length)} selected ${commitLabel} no longer match the current staged patch.`,
    "Use --valid-only to skip mismatches or --force to bypass saved hash validation.",
    ...mismatchLines,
  ].join("\n");
}
