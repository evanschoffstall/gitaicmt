...
export function isDeepNestedInternalUmbrella(groups: PlannedCommit[]): boolean {
  // Use only implementation (non-support) paths for structural analysis so that
  // test files in covered groups do not create spurious extra feature-roots,
  // which would prevent deep-nested umbrella detection from triggering on
  // large commit-planning-only rollouts that happen to carry attached tests.
  const fileDescriptors = groups
    .flatMap((group) =>
    ...
    .filter(({ path }) => !isSupportLikePath(path));
