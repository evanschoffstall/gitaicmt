...
export function rescopeGroupMessageToCoveredGroups(
  ...
  const outputGroup = shouldPreserveOwnerScopedSplitSubject(
    group,
    coveredGroups,
    evaluation.outputGroup,
  )
    ? {
        ...evaluation.outputGroup,
        message: replaceSubjectLine(
          evaluation.outputGroup.message,
          group.message.split("\n")[0] ?? group.message,
        ),
      }
    : evaluation.outputGroup;
...
function countDistinctImplementationOwners(group: PlannedCommit): number {
  return new Set(
    group.files
      .filter((file) => !isSupportLikePath(file.path))
      .map((file) => getPathOwnerDescriptor(file.path).ownerId),
  ).size;
}
...
function shouldPreserveOwnerScopedSplitSubject(
  group: PlannedCommit,
  coveredGroups: PlannedCommit[],
  ...
  const currentSubjectLine = group.message.split("\n")[0] ?? group.message;
  const outputSubjectLine =
    outputGroup.message.split("\n")[0] ?? outputGroup.message;
  if (currentSubjectLine === outputSubjectLine) {
    return false;
  }

  return (
    countDistinctImplementationOwners(group) === 1 &&
    countDistinctImplementationOwners(coveredGroups[0]) > 1
  );
}
