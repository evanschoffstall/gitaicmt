...
const MAX_BALANCED_SURFACE_FILES_PER_OWNER = 4;
...
export function isCompactCrossFeatureEntrypointRollout(
  ...
  return (
    ...
    (isExactCrossFeatureEntrypointRollout(
      shape,
      shallowEntrypointCount,
      directFeatureRoots,
      nestedFeatureRoots,
    ) ||
      isBalancedCrossFeatureEntrypointRollout(
        shape,
        shallowEntrypointCount,
        directFeatureRoots,
        nestedFeatureRoots,
...
function isBalancedCrossFeatureEntrypointRollout(
  shape: ReturnType<typeof buildSurfaceRolloutShape>,
  shallowEntrypointCount: number,
  directFeatureRoots: Set<string>,
  nestedFeatureRoots: Set<string>,
): boolean {
  const coveredFeatureRoots = new Set([
    ...directFeatureRoots,
    ...nestedFeatureRoots,
  ]);

  return (
    coveredFeatureRoots.size === shape.featureRoots.size &&
    directFeatureRoots.size >= 1 &&
    nestedFeatureRoots.size >= 1 &&
    shallowEntrypointCount >= shape.featureRoots.size &&
    shape.surfaceOwnerIds.size <= shape.featureRoots.size + 1 &&
    shape.descriptors.length <=
      shape.surfaceOwnerIds.size * MAX_BALANCED_SURFACE_FILES_PER_OWNER &&
    [...shape.filesPerSurfaceOwner.values()].every(
      (count) => count <= MAX_BALANCED_SURFACE_FILES_PER_OWNER,
    )
  );
}
...
function isExactCrossFeatureEntrypointRollout(
  shape: ReturnType<typeof buildSurfaceRolloutShape>,
  shallowEntrypointCount: number,
  directFeatureRoots: Set<string>,
  nestedFeatureRoots: Set<string>,
