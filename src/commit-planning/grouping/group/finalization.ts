...
import {
  type FinalizeStabilizedContext,
...
export async function finalizePlannedGroups(
  ...
  if (
    ...
    return finalizeStabilizedGroups(
      ...
      current,
      { fileByPath, fileSignals, originalGroups: groups, startedAtMs },
    );
  }
  ...
  return finalizeConsolidatedGroups(groups.length, baselineGroups, current, {
    fileByPath,
    fileSignals,
...
function finalizeConsolidatedGroups(
  ...
  const { fileByPath, fileSignals } = ctx;
