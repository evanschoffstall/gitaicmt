...
/**
 * Narrow escape hatch for exact-narrative single-file tests when broad
 * support attachment is otherwise disabled. The group still has to choose one
 * decisive implementation component first.
 */
export function shouldAttachStandaloneExactDescriptionTestSupport(
  supportGroup: PlannedCommit,
  groups: PlannedCommit[],
  components: number[][],
  fileSignals: Map<string, FileChangeSignals>,
): boolean {
  const supportSubjectLine = supportGroup.message.split("\n")[0] ?? "";
  const supportSubject = parseSubjectWords(supportSubjectLine);
  if (
    supportSubject.type !== "test" ||
    supportGroup.files.length !== 1 ||
    !supportGroup.files.every((file) => isSupportLikePath(file.path))
  ) {
    return false;
  }

  const targetComponentIndex = chooseSupportAttachment(
    supportGroup,
    groups,
    components,
    fileSignals,
  ...
  const supportDescription =
    getNormalizedSubjectDescriptionKey(supportSubjectLine);
  if (supportDescription.length === 0) {
    return false;
  }

  return components[targetComponentIndex].some((groupIndex) => {
    const targetGroup = groups[groupIndex];
    const targetSubjectLine = targetGroup.message.split("\n")[0];
    const targetSubject = parseSubjectWords(targetSubjectLine);

    return (
      targetSubject.type !== "test" &&
      getNormalizedSubjectDescriptionKey(targetSubjectLine) ===
        supportDescription
    );
  });
}
