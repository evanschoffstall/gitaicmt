...
function buildSupportAttachmentTraceDiagnostics(
  ...
  return {
    ...
    hasExactDescriptionSignal: signals.hasExactDescriptionSignal,
...
function getSupportAttachmentSignals(
  ...
  return {
    hasCoverageSignal,
    hasDependencySignal,
    hasExactDescriptionSignal: hasExactDescriptionSignal(
      supportGroup,
      targetGroup,
      supportSubject,
      targetSubject,
...
function getSupportWordOverlapScore(signals: AttachmentSignals): number {
  return (
    (signals.hasExactDescriptionSignal ? 2 : 0) +
...
function hasExactDescriptionSignal(
  supportGroup: PlannedCommit,
  targetGroup: PlannedCommit,
  supportSubject: ReturnType<typeof parseSubjectWords>,
  targetSubject: ReturnType<typeof parseSubjectWords>,
): boolean {
  if (supportSubject.type !== "test" || targetSubject.type === "test") {
    return false;
  }
  ...
  const targetDescriptionKey = getNormalizedSubjectDescriptionKey(
    targetGroup.message.split("\n")[0] ?? "",
  );
  return (
    supportDescriptionKey.length > 0 &&
    supportDescriptionKey === targetDescriptionKey
  );
}
