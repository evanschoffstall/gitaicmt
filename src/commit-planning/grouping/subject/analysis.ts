...
/** Builds a subject key from only the narrative, ignoring type and scope. */
export function getNormalizedSubjectDescriptionKey(subject: string): string {
  const parsed = parseConventionalSubject(subject);
  const descriptionWords = sanitizeSubjectWords(parsed.description);
  const normalizedDescription = descriptionWords.join(" ").trim();
  if (normalizedDescription.length > 0) {
    return normalizedDescription;
  }

  return sanitizeSubjectWords(subject).join(" ").trim();
}

/**
 * Builds a stable merge key for subject-equivalent commits while ignoring scope
 * differences such as `test(cli)` vs `test(git)`.
 */
export function getNormalizedSubjectMergeKey(subject: string): string {
  const parsed = parseConventionalSubject(subject);
  const descriptionWords = sanitizeSubjectWords(parsed.description);
  const normalizedDescription = descriptionWords.join(" ").trim();
  if (normalizedDescription.length === 0) {
    const fallbackWords = sanitizeSubjectWords(subject).join(" ").trim();
    return parsed.type.length > 0
      ? `${parsed.type}:${fallbackWords}`
      : fallbackWords;
  }

  return parsed.type.length > 0
    ? `${parsed.type}:${normalizedDescription}`
