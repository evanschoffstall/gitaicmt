...
function buildPremergePairDiagnostics(
  ...
  return {
    ...
    shouldPremergeSameNormalizedSubject: matchedRuleSet.has(
...
function getPremergeMatchedRules(
  ...
  return [
    shouldPremergeSameNormalizedSubject(leftGroup, rightGroup)
      ? "same-normalized-subject"
      : null,
...
function shouldPremergeSameNormalizedSubject(
  leftGroup: PlannedCommit,
  rightGroup: PlannedCommit,
): boolean {
  const leftSubject = leftGroup.message.split("\n")[0] ?? "";
  const rightSubject = rightGroup.message.split("\n")[0] ?? "";
  const leftKey = getNormalizedSubjectMergeKey(leftSubject);
  const rightKey = getNormalizedSubjectMergeKey(rightSubject);
