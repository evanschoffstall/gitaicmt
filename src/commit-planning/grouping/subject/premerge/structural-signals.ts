...
export function getImplementationAnchoredOwnerGroup(
  group: PlannedCommit,
): PlannedCommit {
  const implementationFiles = group.files.filter(
...
export function hasSharedChangedExportIdentifierFollowUp(
  ...
  return (
    entrypointSubject.words.has("public") &&
    (entrypointSubject.words.has("export") ||
      entrypointSubject.words.has("exports")) &&
    getSharedChangedIdentifiers(entrypointGroup, ownerGroup, fileByPath).size >
      0
  );
}
