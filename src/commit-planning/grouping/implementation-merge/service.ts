...
export function hasImplementationMergeSignal(
  ...
  if (hasExactSameFeatureSurfaceSubjectMerge(left, right)) {
    return true;
  }
...
function hasExactSameFeatureSurfaceSubjectMerge(
  left: PlannedCommit,
  right: PlannedCommit,
): boolean {
  const leftSubject = left.message.split("\n", 1)[0] ?? "";
  const rightSubject = right.message.split("\n", 1)[0] ?? "";

  return (
    leftSubject.length > 0 &&
