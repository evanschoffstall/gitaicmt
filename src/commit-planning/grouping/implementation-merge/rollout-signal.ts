...
import {
  hasCompatibleRolloutOwnership,
  hasStrongRolloutSignalMetadata,
} from "./strong-rollout-compatibility.js";
...
export function hasStrongSharedRolloutMerge(
  ...
  return (
    hasCompatibleRolloutOwnership(left, right) &&
    hasLimitedSameFeatureRolloutStructure(left, right) &&
    hasStrongRolloutSignalMetadata({
      broaderGroupSupported:
        broaderGroup === null || hasEntrypointLikeSurfaceGroup(broaderGroup),
      hasRelatedScopes:
        hasRelatedScopes(mergeSignal) ||
        mergeSignal.leftSubject.scope.length === 0 ||
        mergeSignal.rightSubject.scope.length === 0,
      leftType: mergeSignal.leftSubject.type,
      leftWords: mergeSignal.leftSubject.words,
      rightType: mergeSignal.rightSubject.type,
      rightWords: mergeSignal.rightSubject.words,
      sharedIntentScore: mergeSignal.sharedIntentScore,
      sharedWordCount: mergeSignal.sharedWordCount,
    })
  );
}
