...
export function evictOverflowPlanBundles(cwd?: string): void {
  ...
  const entries = listPlanBundleFilesForRepository(repositoryRoot);
...
/**
 * Return every persisted bundle file in oldest-first save order regardless of
 * repository ownership.
 */
export function listAllPlanBundleFiles(): BundleFileEntry[] {
  const directory = resolvePlanBundleDirectory();
  if (!existsSync(directory)) {
    return [];
  }

  return listPlanBundleFiles(directory);
}

/**
 * Return persisted bundle files for the current repository in oldest-first
 * filesystem order so callers can preserve visible save chronology.
 */
export function listPlanBundleFilesForRepository(
  cwd?: string,
): BundleFileEntry[] {
  const directory = resolvePlanBundleDirectory();
  if (!existsSync(directory)) {
  ...
  const repositoryRoot = resolve(getRepositoryRoot(cwd));
  return listPlanBundleFiles(directory).filter(
