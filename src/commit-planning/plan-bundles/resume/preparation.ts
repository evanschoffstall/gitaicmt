...
function validateRestoredResumeSelection(
  ...
  const normalizedSelectedResumePatch =
    normalizePatchContent(selectedResumePatch);
  const restoredPatch = safeGetStagedPatch(cwd);
  if (normalizedSelectedResumePatch.trim().length === 0) {
    if (restoredPatch.trim().length === 0) {
      return;
    }

    throw new ValidationError(
      `Saved resume selection restored unexpected staged content: expected=${hashContent(normalizedSelectedResumePatch)}, actual=${hashContent(restoredPatch)}.`,
    );
  }
