...
export function buildGroupingUserPrompt(
  ...
  const parts: string[] = [
    ...
    ...buildBatchContextSection(allFiles, files.length, context),
...
function buildBatchContextSection(
  allFiles: FileDiff[],
...
function buildFileLegendSection(
  ...
  return [
    "",
    "File legend:",
    ...files.map((file) => `  ${getFileAlias(fileAliases, file.path)} = ${file.path}`),
  ];
}
