...
| `--enforce-commit-body` | Require bullet-body validation during commit replay or execution |
| `--force`          | Resume selected saved commits without commit-level hash filtering |
| `--valid-only`     | Resume only saved commits whose file or hunk hashes still match |
...
gitaicmt resume <hash> --valid-only   # salvage only matching saved commits
gitaicmt resume <hash> --force   # bypass commit-level hash filtering
...
`plan` and the default commit flow save a hashed plan bundle under the user cache directory. The CLI prints the bundle hash after planning so you can replay the selected saved patch later with `gitaicmt resume <hash>` from the same repository checkout, even if HEAD has moved since the bundle was created.
...
- Resume remains scoped to the same repository root, not to the original saved HEAD.
- Default resume mode is strict and aborts when any selected saved commit hash no longer matches the current staged patch.
- Pass `--valid-only` to execute only the matching saved commit subset.
- Pass `--force` to bypass commit-level hash filtering after the selected saved patch restores successfully.
