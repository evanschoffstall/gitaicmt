...
describe("deterministic pre-merge", () => {
  ...
  test("premergeBySubject keeps support-only identifier follow-ups separate inside one coverage file", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const sharedCoverageFile = {
      ...makeFileDiff("tests/zz-cli-coverage.test.ts", 2, 2),
      hunks: [
        {
          countNew: 1,
          countOld: 1,
          header: "@@ -1,1 +1,1 @@",
          lines: [
            "-expect(validationOptions).toEqual([{ ignoreMessageBody: true }]);",
            "+expect(validationOptions).toEqual([{ enforceMessageBody: true }]);",
          ],
          startNew: 1,
          startOld: 1,
        },
        {
          ...
          header: "@@ -20,1 +20,1 @@",
          lines: [
            "-expect(parseCliOptions(['resume', 'abc'])).toMatchObject({ resumeHash: 'abc' });",
            "+expect(parseCliOptions(['resume', 'list'])).toMatchObject({ resumeCommand: 'list' });",
          ],
          startNew: 20,
          startOld: 20,
        },
    ...
    const fileByPath = new Map([[sharedCoverageFile.path, sharedCoverageFile]]);
    const groups = [
      {
        files: [{ hunks: [0], path: sharedCoverageFile.path }],
        message:
          "fix(cli): cover enforced commit body validation semantics\n\n- Verify explicit body enforcement reaches commit execution.",
      },
      {
        files: [{ hunks: [1], path: sharedCoverageFile.path }],
        message:
          "fix(resume): cover listing and strict mismatch safeguards\n\n- Verify resume list parsing and strict hash mismatch guidance.",
      },
    ];
    ...
    expect(result).toHaveLength(2);
    expect(result.map((group) => group.message.split("\n")[0])).toEqual([
      "fix(cli): cover enforced commit body validation semantics",
      "fix(resume): cover listing and strict mismatch safeguards",
    ]);
  });
  ...
  test("premergeBySubject merges exact-scope CLI help follow-ups into the owning commit-body fix", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const commitExecutionFile = makeFileDiff(
      "src/cli/commit/execution.ts",
      1,
      0,
    );
    const executionFlowFile = makeFileDiff("src/cli/execution-flow.ts", 1, 0);
    const mainFile = makeFileDiff("src/cli/main.ts", 1, 0);
    const optionsFile = makeFileDiff("src/cli/options.ts", 1, 0);
    const helpTestFile = makeFileDiff("tests/cli.test.ts", 1, 0);
    const fileByPath = new Map([
      [commitExecutionFile.path, commitExecutionFile],
      [executionFlowFile.path, executionFlowFile],
      [helpTestFile.path, helpTestFile],
    ...
    const groups = [
      {
        files: [
          { path: commitExecutionFile.path },
          { path: executionFlowFile.path },
          { path: mainFile.path },
          { path: optionsFile.path },
        ],
        message:
          "fix(cli): make body validation opt-in during execution\n\n- Route enforceMessageBody through CLI execution paths.",
      },
      {
        files: [{ path: helpTestFile.path }],
        message:
          "test(cli): update help coverage for commit body validation flag\n\n- Verify help text documents the renamed enforcement flag.",
      },
    ];
  ...
  test("premergeBySubject merges a tiny same-file style follow-up into the owning mixed rollout", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const validationFile = makeFileDiff(
      "src/git/commit-input-validation.ts",
      1,
      0,
    );
    const gitCoverageFile = makeFileDiff("tests/git-coverage.test.ts", 1, 0);
    const cliCoverageFile = makeFileDiff("tests/cli.test.ts", 2, 0);
    const fileByPath = new Map([
      [cliCoverageFile.path, cliCoverageFile],
      [gitCoverageFile.path, gitCoverageFile],
      [validationFile.path, validationFile],
    ]);
    const groups = [
      {
        files: [
          { path: validationFile.path },
          { path: gitCoverageFile.path },
          { hunks: [0], path: cliCoverageFile.path },
        ],
        message:
          "fix(cli): allow subject-only commit messages by default\n\n- Allow subject-only commit messages unless strict body enforcement is requested.",
      },
      {
        files: [{ hunks: [1], path: cliCoverageFile.path }],
        message: "style(cli): remove duplicate mkdirSync import",
      },
    ];
    ...
    expect(result[0]?.files).toEqual([
      { path: validationFile.path },
      { path: gitCoverageFile.path },
      { hunks: [0, 1], path: cliCoverageFile.path },
    ]);
  });

  test("premergeBySubject merges an exact-scope test follow-up back into a mixed-owner fix rollout", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const validationFile = makeFileDiff(
      "src/git/commit-input-validation.ts",
    ...
    const bundleStorageFile = makeFileDiff(
      "src/commit-planning/plan-bundles/storage.ts",
      1,
      0,
    );
    const gitCoverageFile = makeFileDiff("tests/git-coverage.test.ts", 1, 0);
    const cliCoverageFile = makeFileDiff("tests/cli.test.ts", 1, 0);
    const fileByPath = new Map([
      [bundleStorageFile.path, bundleStorageFile],
      [cliCoverageFile.path, cliCoverageFile],
      [gitCoverageFile.path, gitCoverageFile],
      [validationFile.path, validationFile],
    ]);
    const groups = [
      ...
      {
        files: [{ path: cliCoverageFile.path }],
        message:
          "test(cli): allow subject-only commit messages by default\n\n- Verify CLI coverage still accepts subject-only messages by default.",
      },
    ];
    ...
    expect(result[0]?.files).toEqual([
      ...
      { path: bundleStorageFile.path },
      { path: gitCoverageFile.path },
      { path: cliCoverageFile.path },
    ]);
  });

  test("premergeBySubject merges a tiny two-file exact-scope test follow-up into the owning CLI fix rollout", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const commitExecutionFile = makeFileDiff(
      "src/cli/commit/execution.ts",
    ...
    const optionsFile = makeFileDiff("src/cli/options.ts", 1, 0);
    const cliCoverageFile = makeFileDiff("tests/cli.test.ts", 1, 0);
    const cliExecutionCoverageFile = makeFileDiff(
    ...
    const fileByPath = new Map([
      [cliCoverageFile.path, cliCoverageFile],
      [cliExecutionCoverageFile.path, cliExecutionCoverageFile],
      [commitExecutionFile.path, commitExecutionFile],
      [executionFlowFile.path, executionFlowFile],
      [mainFile.path, mainFile],
      [optionsFile.path, optionsFile],
    ...
    const groups = [
      {
        ...
        message:
          "fix(cli): enforce body validation only when requested\n\n- Keep subject-only commits valid unless explicit enforcement is enabled.",
      },
      {
        files: [
          { path: cliCoverageFile.path },
          { path: cliExecutionCoverageFile.path },
        ],
        message:
          "test(cli): make commit body enforcement explicit in execution tests\n\n- Verify CLI and execution coverage only enforce bodies when requested.",
      },
    ];
  ...
  test("premergeBySubject merges a tiny fix-labeled test bucket into the owning commit-body rollout", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const commitExecutionFile = makeFileDiff(
      "src/cli/commit/execution.ts",
    ...
    const groups = [
      {
        ...
        message:
          "fix(commit-body): enforce body validation only when requested\n\n- Keep execution body validation opt-in.",
      },
      {
        files: [
          { path: cliCoverageFile.path },
        ...
        message:
          "fix(cli): make commit body enforcement opt-in in execution paths\n\n- Verify CLI execution tests keep body validation opt-in.",
      },
    ];
  ...
  test("premergeBySubject merges a one-file exact-scope test shard into the low-overlap commit-body rollout", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const commitExecutionFile = makeFileDiff(
      "src/cli/commit/execution.ts",
    ...
    const groups = [
      {
        ...
        message:
          "fix(commit): require bodies only when explicitly requested\n\n- Thread explicit enforcement through commit execution paths.",
      },
      {
        files: [{ path: cliCoverageFile.path }],
        message:
          "test(cli): make commit body validation opt-in\n\n- Verify CLI coverage keeps commit body validation opt-in.",
      },
    ];
  ...
  test("premergeBySubject merges a tiny test follow-up with the exact same narrative across types", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const traceDecisionFile = makeFileDiff(
      "src/cli/trace/decision/aggregation.ts",
      1,
      0,
    );
    const sessionDisplayFile = makeFileDiff("src/cli/session-display.ts", 1, 0);
    const sessionDisplayTestFile = makeFileDiff(
      "tests/session-display-trace-aggregation.test.ts",
      1,
      0,
    );
    const fileByPath = new Map([
      [sessionDisplayFile.path, sessionDisplayFile],
      [sessionDisplayTestFile.path, sessionDisplayTestFile],
      [traceDecisionFile.path, traceDecisionFile],
    ]);
    const groups = [
      {
        files: [
          { path: traceDecisionFile.path },
          { path: sessionDisplayFile.path },
        ],
        message:
          "feat(trace-decision): add aggregated planner and event trace summaries\n\n- Buffer and render compact planner decision summaries.",
      },
      {
        files: [{ path: sessionDisplayTestFile.path }],
        message:
          "test(session-display-trace-aggregation): add aggregated planner and event trace summaries\n\n- Verify session display trace summaries stay aggregated.",
      },
    ];
    ...
    expect(result[0]?.files).toEqual([
      { path: traceDecisionFile.path },
      { path: sessionDisplayFile.path },
      { path: sessionDisplayTestFile.path },
    ]);
  });

  test("premergeBySubject merges a generic export-only follow-up into the identifier-sharing nested rollout", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const listingServiceFile: FileDiff = {
      ...makeFileDiff("src/commit-planning/plan-bundles/service.ts", 1, 0),
      hunks: [
        {
          countNew: 1,
          countOld: 0,
          header: "@@ -1,0 +1,1 @@",
          lines: ["+export function listSavedPlanBundles() {}"],
          startNew: 1,
          startOld: 1,
        },
      ],
    };
    const listingIndexFile: FileDiff = {
      ...makeFileDiff("src/commit-planning/plan-bundles/index.ts", 1, 0),
      hunks: [
        {
          countNew: 1,
          countOld: 0,
          header: "@@ -1,0 +1,1 @@",
          lines: ["+export { listSavedPlanBundles } from './service.js';"],
          startNew: 1,
          startOld: 1,
        },
      ],
    };
    const resumeValidationFile: FileDiff = {
      ...makeFileDiff(
        "src/commit-planning/plan-bundles/resume/validation.ts",
        1,
        0,
      ),
      hunks: [
        {
          ...
          countOld: 0,
          header: "@@ -1,0 +1,1 @@",
          lines: ["+export function validateResumeProgress() {}"],
          startNew: 1,
          startOld: 1,
        },
      ],
    };
    const exportFile: FileDiff = {
      ...makeFileDiff("src/commit-planning/index.ts", 1, 0),
      hunks: [
        {
          countNew: 1,
          countOld: 0,
          header: "@@ -1,0 +1,1 @@",
          lines: [
            "+export { listSavedPlanBundles } from './plan-bundles/index.js';",
          ],
          startNew: 1,
          startOld: 1,
        },
    ...
    const fileByPath = new Map([
      ...
      [listingIndexFile.path, listingIndexFile],
      [listingServiceFile.path, listingServiceFile],
      [resumeValidationFile.path, resumeValidationFile],
    ]);
    const groups = [
      {
        files: [
          { path: listingServiceFile.path },
          { path: listingIndexFile.path },
        ],
        message:
          "feat(resume): list saved plan bundles from the cache\n\n- Expose compact saved-bundle listing for CLI rendering.",
      },
      {
        files: [{ path: resumeValidationFile.path }],
        message:
          "fix(resume): skip already replayed plan commits on retry\n\n- Keep resume validation separate from bundle listing.",
      },
      {
        files: [{ path: exportFile.path }],
        message:
          "fix(commit-planning): update public exports\n\n- Summarize the change.",
      },
    ];
    ...
    expect(result[1]?.files).toEqual([{ path: resumeValidationFile.path }]);
  });
  ...
  test("premergeBySubject merges same normalized subjects across scopes", async () => {
    const { premergeBySubject } =
      await import("../src/commit-planning/grouping/index.js");
    const fileByPath = new Map([
      ["tests/cli.test.ts", makeFileDiff("tests/cli.test.ts", 1, 0)],
      [
        "tests/git-coverage.test.ts",
        makeFileDiff("tests/git-coverage.test.ts", 1, 0),
      ],
    ]);
    const groups = [
      {
        ...
        message:
          "test(cli): add saved bundle listing and strict replay modes\n\n- Cover resume list output.",
      },
      {
        files: [{ path: "tests/git-coverage.test.ts" }],
        message:
          "test(git): add saved bundle listing and strict replay modes\n\n- Cover resume validation behavior.",
      },
    ];
