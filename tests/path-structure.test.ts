...
describe("dynamic path structure helpers", () => {
  ...
  test("resolveKnownPath preserves unique basename salvage after alias registration", () => {
    const resolver = buildFilePathResolver(
      new Map([
        [
    ...
    expect(
      resolveKnownPath(
        "src/commit-planning/grouping/group/follow-up-rules.ts",
        resolver,
      ),
    ).toBe("src/commit-planning/grouping/group/adjacent/follow-up-rules.ts");
  });
