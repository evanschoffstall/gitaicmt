...
describe("ai coverage", () => {
  ...
  test("finalizePlannedGroups drops duplicate export-only groups after stabilization", async () => {
    const { finalizePlannedGroups } = await importFinalizeModule(
      "stabilized-duplicate-export-groups",
    );

    const groups = [
      {
        files: [{ hunks: [0, 1], path: "src/commit-planning/index.ts" }],
        message: commitMessage("fix(commit-planning): update public exports"),
      },
      {
        files: [{ hunks: [0, 1], path: "src/commit-planning/index.ts" }],
        message: commitMessage("fix(commit-planning): update public exports"),
    ...
    const allFiles = [makeFile("src/commit-planning/index.ts", 2)];
  ...
  test("finalizePlannedGroups splits disconnected consolidation intents back apart", async () => {
    ...
    expect(result[0]?.message).toContain("refactor(");
    expect(result[0]?.message).toContain("merge");
    expect(result[1]).toEqual(groups[1]);
    expect(result[2]?.files).toEqual([{ path: "tests/ai.test.ts" }]);
  ...
  test("absorbIncidentalAdjacentGroups merges an exact-narrative test follow-up into an adjacent feature rollout", async () => {
    const { absorbIncidentalAdjacentGroups } = await import(
      new URL(
        `../src/commit-planning/grouping/group/index.js?adjacent-exact-narrative-test-follow-up-${Math.random()}`,
        import.meta.url,
      ).href
    );
    ...
    const groups = [
      {
        ...
        message: commitMessage(
          "feat(trace-decision): summarize planner decisions in trace output",
          "- Aggregate planner decision summaries before rendering the session display.",
        ),
      },
      {
        files: [{ path: "tests/session-display-trace-aggregation.test.ts" }],
        message: commitMessage(
          "test(session-display-trace-aggregation): summarize planner decisions in trace output",
          "- Verify session-display trace aggregation summaries render before the plan summary.",
        ),
      },
    ];
    const files = [
    ...
    expect(result[0]?.files).toEqual([
      { path: "src/cli/trace/decision/aggregation.ts" },
      { path: "src/cli/session-display.ts" },
      { path: "tests/session-display-trace-aggregation.test.ts" },
    ]);
    expect(result[0]?.message.split("\n")[0]).toBe(
      "feat(trace-decision): summarize planner decisions in trace output",
    );
  ...
  test("splitWeakConsolidations distributes broad support across multiple decisive owners", async () => {
    const { splitWeakConsolidations } = await import(
      new URL(
        `../src/commit-planning/grouping/repartition.js?multi-owner-support-attachment-${Math.random()}`,
        import.meta.url,
      ).href
    );
    const { buildFileChangeSignals } = await import(
      new URL(
        `../src/commit-planning/grouping/file/index.js?multi-owner-support-attachment-${Math.random()}`,
        import.meta.url,
      ).href
    );
    ...
    const groups = [
      {
        ...
        message: commitMessage(
          "feat(trace-decision): track completed saved-plan replay progress",
          "- Keep trace-decision replay summaries visible in the session display.",
        ),
      },
      {
        files: [
        ...
        message: commitMessage(
          "feat(resume): track completed saved-plan replay progress",
          "- Keep saved-plan replay progress and resume controls aligned.",
        ),
      },
      {
        files: [
        ...
        message: commitMessage(
          "test(runtime): track completed saved-plan replay progress",
          "- Verify session-display trace summaries and resume CLI replay progress together.",
        ),
      },
    ];
    const allFiles = [
      ...
      makeFile("src/cli/resume/options.ts"),
      makeFile("tests/session-display-trace-aggregation.test.ts"),
      makeFile("tests/cli.test.ts"),
    ];
      ...
      [
        {
          files: allFiles.map((file) => ({ path: file.path })),
          message: commitMessage(
            "feat(runtime): track completed saved-plan replay progress",
            "- Combine trace display, resume replay, and broad runtime coverage in one umbrella rollout.",
          ),
        },
      ],
      new Map(allFiles.map((file) => [file.path, file])),
    ...
    const traceDecisionGroup = result.find((group: PlannedCommit) =>
      group.files.some(
        (file: PlannedCommit["files"][number]) =>
          file.path === "src/cli/trace/decision/aggregation.ts",
      ),
    );
    const resumeGroup = result.find((group: PlannedCommit) =>
      group.files.some(
        (file: PlannedCommit["files"][number]) =>
          file.path === "src/cli/resume/execution.ts",
      ),
    );

    expect(traceDecisionGroup).toBeDefined();
    expect(resumeGroup).toBeDefined();
    expect(
      traceDecisionGroup?.files.some(
        (file: PlannedCommit["files"][number]) =>
          file.path === "tests/session-display-trace-aggregation.test.ts",
      ),
    ).toBe(true);
    expect(
      resumeGroup?.files.some(
        (file: PlannedCommit["files"][number]) =>
    ...
    expect(
      ...
      expect.arrayContaining([
        "test(session-display-trace-aggregation): track completed saved-plan replay progress",
        "test(cli): track completed saved-plan replay progress",
      ]),
    );
  });
  ...
  test("splitWeakConsolidations attaches a single-file exact-narrative test shard even when compact support attachment is disabled", async () => {
    const { splitWeakConsolidations } = await import(
      new URL(
        `../src/commit-planning/grouping/repartition.js?single-file-exact-description-support-${Math.random()}`,
        import.meta.url,
      ).href
    );
    const { buildFileChangeSignals } = await import(
      new URL(
        `../src/commit-planning/grouping/file/index.js?single-file-exact-description-support-${Math.random()}`,
        import.meta.url,
      ).href
    );
    ...
    const groups = [
      {
        ...
        message: commitMessage(
          "feat(trace-decision): add aggregated planner and event summaries",
          "- Aggregate planner decisions before rendering the session display.",
        ),
      },
      {
        files: [{ path: "src/cli/resume/execution.ts" }],
        message: commitMessage(
          "feat(resume): list saved bundles and resume across HEAD changes",
          "- Keep resume execution aligned with saved bundle listing.",
        ),
      },
      {
        files: [{ path: "tests/session-display-trace-aggregation.test.ts" }],
        message: commitMessage(
          "test(session-display-trace-aggregation): add aggregated planner and event summaries",
          "- Verify trace summaries render before the plan summary.",
        ),
      },
    ];
    const allFiles = [
    ...
    expect(result[1]?.files).toEqual([{ path: "src/cli/resume/execution.ts" }]);
  });
  ...
  test("finalizePlannedGroups rejects a repeated prompt-builder umbrella family and restores structural ownership slices", async () => {
    writeLocalConfig({
      openai: {
      ...
      result.some(
        (group: PlannedCommit) =>
          group.files.some(
            (file: PlannedCommit["files"][number]) =>
              file.path ===
              "src/commit-planning/prompt-builders/commit-format.ts",
          ) &&
          group.files.some(
            (file: PlannedCommit["files"][number]) =>
    ...
    expect(result[0]?.files).toHaveLength(7);
    expect(events.length).toBeGreaterThan(0);
    observer.setAiOutputObserver(null);
  });

  test("finalizePlannedGroups merges same-subject groups even when they span multiple ownership roots", async () => {
    writeLocalConfig({
      openai: {
    ...
    expect(calls.chat.length).toBeLessThanOrEqual(1);
    // All three input groups share the same normalized subject description.
    // They must be merged into one commit regardless of scope differences.
    expect(result).toHaveLength(1);
  });

  test("finalizeWithoutConsolidation merges same normalized subjects across scopes", async () => {
    const { finalizeWithoutConsolidation } = await import(
      new URL(
        `../src/commit-planning/grouping/group/group-stability.js?same-subject-merge-no-consolidation-${Math.random()}`,
        import.meta.url,
      ).href
    );
    const { buildFileChangeSignals } = await import(
      new URL(
        `../src/commit-planning/grouping/file/index.js?same-subject-merge-no-consolidation-${Math.random()}`,
        import.meta.url,
      ).href
    );
    ...
    const groups: PlannedCommit[] = [
      {
        ...
        message: commitMessage(
          ...
          "- Cover resume list command behavior.",
        ),
      },
      {
        ...
        message: commitMessage(
          ...
          "- Cover resume replay filtering behavior.",
        ),
      },
    ];
    const allFiles = [
    ...
    const result = finalizeWithoutConsolidation(
      groups,
      new Map(allFiles.map((file) => [file.path, file])),
      buildFileChangeSignals(allFiles),
      groups.length,
