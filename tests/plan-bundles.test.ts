...
function createShiftedSameFileSplitPlan(): PlannedCommit[] {
  return [
    {
      files: [{ hunks: [0], path: "app.ts" }],
      message: "feat(test): keep the leading app hunk",
    },
    {
      files: [{ hunks: [1], path: "app.ts" }],
      message: "feat(test): keep the trailing app hunk",
    },
  ];
}
...
describe("plan bundles", () => {
  ...
  test("skips previously completed resume commits when the recorded replay HEAD still matches", () => {
    const directory = createGitRepo();
    writeFileSync(join(directory, "first.txt"), "first\n", "utf-8");
    writeFileSync(join(directory, "second.txt"), "second\n", "utf-8");
    execSync("git add first.txt second.txt", {
    ...
    execSync("git commit -m 'feat(test): keep first staged file'", {
      cwd: directory,
      stdio: "pipe",
    });
    ...
    const reloadedBundle = loadPlanBundle(savedBundle.hash);
    markPlanBundleResumeCommitCompleted(reloadedBundle, 1, directory);
    const pendingIndexes = filterPendingResumeIndexes(
      reloadedBundle,
      [1, 2],
      directory,
    );

    expect(pendingIndexes).toEqual([2]);
    expect(() =>
      preparePlanBundleForResume(reloadedBundle, pendingIndexes, directory),
    ).not.toThrow();
    expect(getStagedPatch(directory)).toBe(remainingPatch);
  });

  test("infers already replayed leading resume commits from recent HEAD messages when no progress sidecar exists", () => {
    const directory = createGitRepo();
    writeFileSync(join(directory, "first.txt"), "first\n", "utf-8");
    writeFileSync(join(directory, "second.txt"), "second\n", "utf-8");
    execSync("git add first.txt second.txt", {
  ...
  test("restores a remaining same-file resume hunk even when HEAD movement changes the raw patch text", () => {
    const directory = createGitRepo();
    writeFileSync(
    ...
    execSync("git add app.ts", {
      cwd: directory,
      stdio: "pipe",
    });
    execSync("git commit -m 'seed app fixture'", {
      cwd: directory,
      stdio: "pipe",
    });
    ...
    const stagedPatch = getStagedPatch(directory);
    expect(parseDiff(stagedPatch)[0]?.hunks).toHaveLength(2);

    const savedBundle = savePlanBundle(
      createShiftedSameFileSplitPlan(),
      stagedPatch,
      directory,
    );
    const persistedBundle = loadPlanBundle(savedBundle.hash);

    execSync("git reset HEAD -- .", {
      cwd: directory,
      stdio: "pipe",
    });
    restoreStagedPatch(persistedBundle.planCommitPatches[0] ?? "", directory);
    execSync("git commit -m 'feat(test): keep the leading app hunk'", {
      cwd: directory,
      stdio: "pipe",
    });
